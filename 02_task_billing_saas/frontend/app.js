...
function App() {
    const [tasks, setTasks] = useState([
        { id: 1, title: "FastAPI エンドポイント設計", project_name: "ECリニューアル", hourly_rate: 6500, status: "done", priority: "high" },
        { id: 2, title: "Stripe 決済連携シミュレータ", project_name: "ECリニューアル", hourly_rate: 6500, status: "in_progress", priority: "high" },
        { id: 3, title: "GSAP スクロール演出調整", project_name: "ブランドLP制作", hourly_rate: 5500, status: "todo", priority: "medium" }
    ]);
    const [timeLogs, setTimeLogs] = useState([
        { id: 1, project_name: "ECリニューアル", duration_seconds: 7200, notes: "API設計レビュー", created_at: "2026-08-29" },
        { id: 2, project_name: "ブランドLP制作", duration_seconds: 10800, notes: "Heroセクション実装", created_at: "2026-08-29" }
    ]);
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
            ...
            <main className="flex-1 max-w-6xl w-full mx-auto px-6 py-8">
                ...
                {currentView === "invoice" && (
                    <div className="max-w-3xl mx-auto bg-white p-10 rounded-3xl border border-slate-200 shadow-lg space-y-8">
                        <div className="flex justify-between items-start border-b border-slate-200 pb-6">
                            <div>
                                <h2 className="text-3xl font-extrabold font-serif text-slate-900">御 請 求 書</h2>
                                <p className="text-xs text-slate-400 font-mono mt-1">請求番号: INV-202608-001</p>
                            </div>
                            <div className="text-right text-xs text-slate-600 space-y-1">
                                <div className="font-bold text-slate-900">鈴木 一郎 エンジニアリング</div>
                                <div>発行日: 2026年8月29日</div>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 flex justify-end">
                            <button onClick={() => window.print()} className="px-6 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs flex items-center space-x-2">
